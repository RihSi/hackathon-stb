        ...
        <v-tabs-items v-model="tabs">
          <v-tab-item>
            <v-card flat>
              ...
              <v-card-text>
                1.大輪盤獎池：外圈輪盤轉動，最後停止位置即為中獎位置。每局必開一個位置，位置1到10的中獎機率相同。
                  <br><br>
                2.小輪盤獎池：外圈輪盤轉動，最後停止位置即為中獎位置。每局必開一個位置，位置1到10的中獎機率依照購買位置費用成等比級距。
                  <br><br>
                3.寶箱獎池-錢袋：輪盤外圍有閃爍寶箱，點擊後如為錢袋即為中獎。非每局必開出，單局有機率同時多開中獎，獲得獎金為該位置的門票費用。
                  <br><br>
                4.寶箱獎池-JP金幣寶箱：輪盤外圍有閃爍寶箱，點擊後如為金幣寶箱即為中獎。非每局必開出，單局只會有一個位置中獎，獲得獎金為Jackpot累積獎金，如沒有開出則獎金累積至下局。
              </v-card-text>
              <v-card-title>
                彩池獎金分配
              </v-card-title>
          ...
          <v-tab-item>
            <v-card flat>
              ...
              <v-card-text>
                  當最後一個位置被購買之後，合約觸發開獎產生隨機數依照最後一筆購買位置的Transation Hash進行運算。
              </v-card-text>
              <v-card-title>
                <span class="headline">Hash運算</span>
              </v-card-title>
              <v-card-text>
                取Transaction Hash前39個字串，每三個字串為一組開獎結果總共13組，將每組開獎結果進行16進位轉10進位運算，再除以最大值4096，計算後的數字將落在0~1之間，再依照各池的權重判斷分別是落在哪個區間範圍，即為中獎結果。
              </v-card-text>
              <v-card-title>
